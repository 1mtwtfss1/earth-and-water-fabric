        ...
        if (!DarkDripstoneHelper.canGenerate(structureWorldAccess, blockPos)) {
            ...
            if (!optional.isEmpty() && optional.get() instanceof Column.Range) {
                ...
                if (bounded.height() < 4) {
                    ...
                    int j = Mth.clamp(i, largeDarkDripstoneFeatureConfig.columnRadius.getMinValue(), largeDarkDripstoneFeatureConfig.columnRadius.getMaxValue());
                    int k = Mth.randomBetweenInclusive(random, largeDarkDripstoneFeatureConfig.columnRadius.getMinValue(), j);
